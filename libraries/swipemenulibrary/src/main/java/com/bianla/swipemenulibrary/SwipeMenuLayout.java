package com.bianla.swipemenulibrary;
...
public class SwipeMenuLayout extends ViewGroup {
    ...
    private static SwipeMenuLayout mViewCache; //缓存上一个侧滑的View  用于关闭之前打开的View
        ...
        try {
            ...
            for (int i = 0; i < indexCount; i++) {
                ...
                if (attr == R.styleable.SwipeMenuLayout_leftMenuView) {
                    mLeftViewResID = typedArray.getResourceId(R.styleable.SwipeMenuLayout_leftMenuView, -1);
                } else if (attr == R.styleable.SwipeMenuLayout_rightMenuView) {
                    mRightViewResID = typedArray.getResourceId(R.styleable.SwipeMenuLayout_rightMenuView, -1);
    ...
    private State isShouldOpen() {
        ...
        Log.i(TAG, ">>>finalDistanceX:" + finalDistanceX);
        if (finalDistanceX < 0) {
            //➡滑动
            //1、展开左边按钮
            //获得leftView的测量长度
