...
import kotlinx.android.synthetic.main.widget_super_layout.view.*
...
class SuperLayout @JvmOverloads constructor(context: Context, attributeSet: AttributeSet? = null, defStyleAttr: Int = 0)
    ...
    init {
        ...
        leftImage = ta.getDrawable(R.styleable.SuperLayout_widget_sl_leftImageSrc)
        leftImageSize = ta.getDimensionPixelSize(R.styleable.SuperLayout_widget_sl_leftImageSize, leftImageSize)

        leftText = ta.getString(R.styleable.SuperLayout_widget_sl_leftText) ?: ""
        leftTextColor = ta.getColor(R.styleable.SuperLayout_widget_sl_leftTextColor, leftTextColor)
        leftTextSize = ta.getDimensionPixelSize(R.styleable.SuperLayout_widget_sl_leftTextSize, leftTextSize)
        leftTextMarginLeft = ta.getDimensionPixelSize(R.styleable.SuperLayout_widget_sl_leftTextMarginLeft, leftTextMarginLeft)
        leftTextMarginRight = ta.getDimensionPixelSize(R.styleable.SuperLayout_widget_sl_leftTextMarginRight, leftTextMarginRight)
        leftTextMarginTop = ta.getDimensionPixelSize(R.styleable.SuperLayout_widget_sl_leftTextMarginTop, leftTextMarginTop)
        leftTextMarginBottom = ta.getDimensionPixelSize(R.styleable.SuperLayout_widget_sl_leftTextMarginBottom, leftTextMarginBottom)

        leftSubText = ta.getString(R.styleable.SuperLayout_widget_sl_leftSubText) ?: ""
        leftSubTextColor = ta.getColor(R.styleable.SuperLayout_widget_sl_leftSubTextColor, leftSubTextColor)
        leftSubTextSize = ta.getDimensionPixelSize(R.styleable.SuperLayout_widget_sl_leftSubTextSize, leftSubTextSize)

        centerText = ta.getString(R.styleable.SuperLayout_widget_sl_centerText) ?: ""
        centerTextColor = ta.getColor(R.styleable.SuperLayout_widget_sl_centerTextColor, centerTextColor)
        centerTextSize = ta.getDimensionPixelSize(R.styleable.SuperLayout_widget_sl_centerTextSize, centerTextSize)
        centerTextBg = ta.getDrawable(R.styleable.SuperLayout_widget_sl_centerTextBg)

        rightText = ta.getString(R.styleable.SuperLayout_widget_sl_rightText) ?: ""
        rightTextColor = ta.getColor(R.styleable.SuperLayout_widget_sl_rightTextColor, rightTextColor)
        rightTextSize = ta.getDimensionPixelSize(R.styleable.SuperLayout_widget_sl_rightTextSize, rightTextSize)
        rightTextBg = ta.getDrawable(R.styleable.SuperLayout_widget_sl_rightTextBg)
        rightTextBgColor = ta.getColor(R.styleable.SuperLayout_widget_sl_rightTextBgColor, rightTextBgColor)
        rightTextWidth = ta.getDimensionPixelSize(R.styleable.SuperLayout_widget_sl_rightTextWidth, rightTextWidth)
        rightTextHeight = ta.getDimensionPixelSize(R.styleable.SuperLayout_widget_sl_rightTextHeight, rightTextHeight)

        rightImage = ta.getDrawable(R.styleable.SuperLayout_widget_sl_rightImageSrc)
        rightImageSize = ta.getDimensionPixelSize(R.styleable.SuperLayout_widget_sl_rightImageSize, rightImageSize)
        rightImageMarginLeft = ta.getDimensionPixelSize(R.styleable.SuperLayout_widget_sl_rightImageMarginLeft, rightImageMarginLeft)

        rightImage2 = ta.getDrawable(R.styleable.SuperLayout_widget_sl_rightImage2Src)
        rightImage2Size = ta.getDimensionPixelSize(R.styleable.SuperLayout_widget_sl_rightImage2Size, rightImage2Size)
        rightImage2MarginLeft = ta.getDimensionPixelSize(R.styleable.SuperLayout_widget_sl_rightImage2MarginLeft, rightImage2MarginLeft)

        solid = ta.getColor(R.styleable.SuperLayout_widget_sl_solid, solid)
        stroke = ta.getColor(R.styleable.SuperLayout_widget_sl_stroke, stroke)
        strokeWidth = ta.getDimensionPixelSize(R.styleable.SuperLayout_widget_sl_strokeWidth, strokeWidth)
        corner = ta.getDimensionPixelSize(R.styleable.SuperLayout_widget_sl_corner, corner)

        topLineColor = ta.getColor(R.styleable.SuperLayout_widget_sl_topLineColor, topLineColor)
        bottomLineColor = ta.getColor(R.styleable.SuperLayout_widget_sl_bottomLineColor, bottomLineColor)
        lineSize = ta.getColor(R.styleable.SuperLayout_widget_sl_lineSize, lineSize)
        enableRipple = ta.getBoolean(R.styleable.SuperLayout_widget_sl_enableRipple, enableRipple)
        rippleColor = ta.getColor(R.styleable.SuperLayout_widget_sl_rippleColor, rippleColor)

        ta.recycle()
        inflate(context, R.layout.widget_super_layout, this)
        applyAttr()
        applySelf()
    }
